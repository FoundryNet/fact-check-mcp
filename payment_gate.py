"""Pay-per-query gate for fact-check's paid tools — on-chain USDC micropayments on
Solana, fronted by a daily free tier. The price VARIES per tool (verify_claim
$0.02, source_check $0.01, batch_verify $0.01/claim min $0.05, daily_brief $5), so
the price is threaded through the gate (via config.TOOL_PRICES) and bound into the
payment memo.
...
logger = logging.getLogger("fact.pay")
